...
interface Problem {
  ...
  description: string;
  solution: string;
  category: 'creative' | 'campaign' | 'audience' | 'technical';
...
const INSIDE_SALES_B2C_PROBLEMS: Problem[] = [
  {
    id: 1,
  ...
  },
  {
    ...
    solution: 'Segmentação por públicos lookalike de clientes convertidos',
    category: 'audience',
    severity: 'high'
  },
  {
  ...
  {
    id: 5,
    title: 'Falta de Social Proof',
    description: 'Anúncios sem elementos de prova social',
    solution: 'Use social proof nos anúncios (depoimentos, números de clientes)',
    category: 'creative',
    severity: 'medium'
  },
  {
    ...
    solution: 'Implemente pixel de conversão para melhor otimização automática',
    category: 'technical',
    severity: 'high'
  }
];

const InsideSalesB2C: React.FC = () => {
  ...
  const analyzeProblems = (): Problem[] => {
    ...
    if (metrics.meta.staticCTR < BENCHMARK_METRICS.meta.staticCTR) {
      problems.push(
        INSIDE_SALES_B2C_PROBLEMS.find(p => p.title === 'Baixo Engajamento') ||
        INSIDE_SALES_B2C_PROBLEMS[0]
      );
    }
    if (metrics.meta.videoCTR < BENCHMARK_METRICS.meta.videoCTR) {
      problems.push(
        INSIDE_SALES_B2C_PROBLEMS.find(p => p.title === 'Criativos Desgastados') ||
        INSIDE_SALES_B2C_PROBLEMS[1]
      );
    }
    ...
    if (metrics.google.searchCTR < BENCHMARK_METRICS.google.searchCTR) {
      problems.push(
        INSIDE_SALES_B2C_PROBLEMS.find(p => p.category === 'campaign') ||
        INSIDE_SALES_B2C_PROBLEMS[3]
      );
    }
    if (metrics.google.pmaxCTR < BENCHMARK_METRICS.google.pmaxCTR) {
      problems.push(
        INSIDE_SALES_B2C_PROBLEMS.find(p => p.category === 'audience') ||
        INSIDE_SALES_B2C_PROBLEMS[2]
      );
    }

    // Verifica taxa de comparecimento em reuniões
    const meetingRate = (metrics.funnel.meetings / metrics.funnel.opportunities) * 100;
    if (meetingRate < 50) { // Se menos de 50% das oportunidades viram reuniões
      problems.push(
        INSIDE_SALES_B2C_PROBLEMS.find(p => p.category === 'campaign') ||
        INSIDE_SALES_B2C_PROBLEMS[3]
      );
    ...
    // Verifica custo por lead do PMax
    const searchCPL = metrics.google.spend / metrics.funnel.leads;
    const pmaxCPLThreshold = searchCPL * BENCHMARK_METRICS.google.pmaxCPLMultiplier;
    if (metrics.google.pmaxCPL > pmaxCPLThreshold) {
      problems.push(
        INSIDE_SALES_B2C_PROBLEMS.find(p => p.category === 'audience') ||
        INSIDE_SALES_B2C_PROBLEMS[2]
      );
    ...
    <form onSubmit={handleMetricsSubmit} className="max-w-2xl mx-auto">
      ...
      <div className="space-y-6">
        ...
        <div>
          ...
          <input
            type="number"
            value={metrics.funnel.meetings}
            onChange={(e) => handleMetricsChange('funnel', 'meetings', e.target.value)}
            className="block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500"
            required
          />
        </div>
  ...
  const renderAnalysis = () => {
    ...
    const oppToMeetingRate = calculateConversionRate(metrics.funnel.meetings, metrics.funnel.opportunities);
    const meetingToSaleRate = calculateConversionRate(metrics.funnel.sales, metrics.funnel.meetings);
    const overallRate = calculateConversionRate(metrics.funnel.sales, metrics.funnel.leads);

    return (
      <div className="max-w-4xl mx-auto">
        <h2 className="text-2xl font-bold text-gray-900 mb-6">
          Análise do Funil Inside Sales B2C
        </h2>
        ...
        <div className="bg-white rounded-xl shadow-lg p-6 mb-8">
          ...
          <div className="grid md:grid-cols-4 gap-6">
            ...
            <div className="text-center">
              ...
              <div className="text-2xl font-bold text-gray-900">{metrics.funnel.meetings}</div>
              <div className="text-sm text-gray-600">Reuniões ({oppToMeetingRate})</div>
            </div>
            ...
            <div className="text-center">
              ...
              <div className="text-sm text-gray-600">Vendas ({meetingToSaleRate})</div>
            </div>
          </div>
        </div>
